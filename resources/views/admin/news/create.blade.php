...
<div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-divider/30 pb-10">
        <div class="space-y-3">
            ...
            <h1 class="text-4xl font-black text-white tracking-tighter font-display uppercase italic leading-none">Create News Article</h1>
            <div class="flex items-center gap-4">
                <p class="text-xs font-bold text-text-tertiary uppercase tracking-widest opacity-40">New Editorial Session</p>
                <div class="w-1 h-1 rounded-full bg-divider"></div>
                <span class="text-[10px] font-black text-text-tertiary uppercase tracking-[0.3em] opacity-60">Session: Local Cache</span>
            </div>
        </div>
    ...
    <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data" x-ref="mainForm">
        @csrf
